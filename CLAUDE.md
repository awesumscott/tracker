...
- **Merge model**: `log.jsonl` union-merges across parallel writers (git `merge=union`). Events must stay idempotent and disjoint-task-commutative on fold; the acyclic invariant is re-checked on every load (`checkAcyclic`), not just on append, because a merge can introduce a cycle neither side had. Union-merge is a *git* property and does nothing for two processes appending to one file in one tree — the fan-out pattern aims every lane's incidental filings at the main checkout's one log, so `persistAppend` takes an exclusive advisory lock on `log.jsonl` around its length-then-pwrite (01M2V2NPA); never drop it, and keep the concurrent-writer arm in store_test.zig — every other test is single-writer and the defect was invisible to all of them. Never add `merge=union` to `snapshot.jsonl` — it's a whole-file baseline, safe only because `compact` is serialized.
...
- **Merge attributes ship inside the store**: `trk init` writes `.tracker/.gitattributes` (`store.gitattributes_text`) — `log.jsonl`/`tombstones.jsonl` `merge=union`, `snapshot.jsonl`/`quarantine.jsonl` `merge=text`. Keep it INSIDE `.tracker/`, never at the repo root: git resolves attributes per directory and the nearest file wins, so a later root-level `*.jsonl` glob can't capture the baselines. `init` never rewrites an existing `.gitattributes`, so a pin added to the template after a store exists can never reach it that way (01M2N0QW2) — `compact` and `tombstones --rebuild` (the two verbs that write `tombstones.jsonl`, and between them cover every writer of every pinned file) warn instead when a pin is missing; neither can check what's actually in effect (std-only, never shells out to git for this), so the warning only ever claims what trk sees in its own file.
...
- **Help routing**: `--help`/`-h` anywhere in a verb's args must route to help *before* dispatch (positional-title verbs would otherwise mint junk tasks named "--help"). Every dispatched verb needs a `Cli.verbs` entry (dispatch reads the table, so there is no other way to add one), and a test asserts each answers `--help`.
- **Store-root discovery is worktree-bounded; `TRK_READONLY` is the hard stop.** `discover.findRoot` (`src/discover.zig`) never walks past a linked git worktree's root (its `.git` is a plain FILE, never a directory — the on-disk marker, no git binary needed) into an enclosing repo's tracker, even when the worktree lacks its own `.tracker`. But a `.claude/worktrees/<id>/` dir that isn't a *real* git worktree (no `.git` at all — e.g. a scratch/tool dir) has no boundary marker to stop at, so discovery legitimately (if dangerously, for an agent context) climbs into whatever `.tracker` sits above it. `TRK_READONLY=1` in the env is the structural fix for that case: `Cli.read_only` refuses every mutating verb (`Verb.mutates`, plus `doc`'s `mutating_subcommands` `set`/`unset`, via `Cli.isMutating`) before dispatch, independent of which root discovery resolved to.
