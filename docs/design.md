...
Enix consumer repo). `archive_out` names exactly one destination, but a consumer can have a *ruled* split: Enix
records QEMU-gated adoption work in one changelog and prism-library work — gated by host tests plus a
cross-build, not QEMU — in a second, so that each changelog's contents match the gate that verified them. Before
...
(`--tag prism-lib --out annex/prism/CHANGELOG.md`, then a second call for the rest).* Cheapest to build, and
